    ...
    if 'mc_10th_percentile' in results_df.columns and results_df['mc_10th_percentile'].sum() > 0:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate='Median (50th): $%{y:,.0f}<extra></extra>',
                showlegend=True
            ),
            row=1, col=1
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=results_df['owner_age'],
                y=results_df['mc_25th_percentile'],
                name='Monte Carlo 25th %ile',
                line=dict(color='rgba(255, 165, 0, 0.5)', width=1.5, dash='dashdot'),
                hovertemplate='25th percentile: $%{y:,.0f}<extra></extra>',
                showlegend=True
            ),
            row=1, col=1
        )
        ...
        with st.spinner("Running projections (529 + retirement)..."):
            ...
            if enable_monte_carlo:
                ...
                if mc_results and 'year_by_year_percentiles' in mc_results:
                    # Add columns for MC percentiles (conservative: 10th, 25th, median)
                    results_df['mc_10th_percentile'] = 0.0
                    results_df['mc_25th_percentile'] = 0.0
                    ...
                    for idx, row in results_df.iterrows():
                        ...
                        if year in mc_results['year_by_year_percentiles']:
                            ...
                            results_df.at[idx, 'mc_25th_percentile'] = percentiles['25th']
    ...
    if 'results_df' in st.session_state:
        ...
        with col2:
            ...
            if mc_results and 'mc_10th_percentile' in results_df.columns and results_df['mc_10th_percentile'].sum() > 0:
                st.info(f"""
                    **📊 Understanding the Monte Carlo Projections (Conservative View):**
                    ...
                    - **Dash-dot orange line** = 25th percentile (conservative scenario - 75% of outcomes are better)
                    - **Dotted orange line** = 10th percentile (pessimistic scenario - 90% of outcomes are better)
                    - **Orange shaded area** = Range between 10th and 25th percentile (showing downside risk)
                    
                    The Monte Carlo bands show the **impact of market volatility** on your retirement plan. 
                    This view focuses on **conservative scenarios** to help you plan for downside risk.
        ...
        if 'mc_10th_percentile' in results_df.columns and results_df['mc_10th_percentile'].sum() > 0:
            display_columns.extend(['mc_10th_percentile', 'mc_25th_percentile', 'mc_50th_percentile'])
        ...
        if 'mc_10th_percentile' in display_df.columns:
            display_df = display_df.rename(columns={
                ...
                'mc_25th_percentile': 'MC 25th %ile',
        ...
        format_columns = ['total_balance', 'posttax_balance', 'pretax_balance', 'education_balance',
                         ...
                         'MC 10th %ile', 'MC 25th %ile', 'MC Median']
        ...
        if 'MC 10th %ile' in display_df.columns:
            st.info("""
                **💡 Table includes Monte Carlo percentiles (Conservative View):**  
                The table shows the deterministic projection plus Monte Carlo simulation results focusing on conservative scenarios. 
                Compare `total_balance` (single scenario) with `MC Median`, `MC 25th %ile`, and `MC 10th %ile` 
                to understand downside risk and the range of possible outcomes accounting for market volatility.
            """)
